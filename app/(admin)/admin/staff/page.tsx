...
export default async function AdminStaffPage() {
  ...
  const mappedStaff = staff.map((s) => ({
    ...
    role: s.role,
    branchId: s.branchId,
    branchLabel: s.branch?.label || "All Branches",
  ...
  return (
    <div className="min-h-screen bg-background text-white flex flex-col">
      ...
      <main className="max-w-6xl w-full mx-auto px-4 sm:px-6 py-5 sm:py-8 flex-1 space-y-4 sm:space-y-6">
        <div>
          <h1 className="text-xl sm:text-2xl font-black uppercase text-white tracking-tight">STAFF &amp; BRANCHES</h1>
          <p className="text-[11px] sm:text-xs text-zinc-400 mt-0.5 sm:mt-1">Create gym staff logins and assign floor access across the 4 Dubai branches.</p>
        </div>
